;;; dotcl.lisp — Micros (Swank) backend for dotcl
;;;
;;; This file implements the micros/backend interfaces using
;;; dotcl-socket, dotcl-thread, and dotcl-gray contrib modules.
;;;
;;; Load order: after micros backend/backend.lisp, before backend/gray.lisp

(defpackage micros/dotcl
  (:use cl micros/backend micros/source-path-parser micros/source-file-cache))

(in-package micros/dotcl)

;; dotcl-socket / dotcl-thread / dotcl-gray are pulled in via micros.asd's
;; #+dotcl :depends-on, so no explicit (require ...) is needed here.
